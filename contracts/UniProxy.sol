...
contract UniProxy is ReentrancyGuard {
  ...
  function deposit(
    ...
    require(
      clearance.clearDeposit(deposit0, deposit1, msg.sender, to, pos, minIn),
      "deposit not cleared"
    );
  ...
  /// @notice Get the amount of pair token to deposit for the given amount
  /// @param pos Hypervisor Address
  /// @param token Address of token to deposit
  /// @param _deposit Amount of token to deposit
  /// @return amountStart Minimum amount of the pair token to deposit
  /// @return amountEnd Maximum amount of the pair token to deposit
  function getDepositAmount(
    address pos,
    address token,
    uint256 _deposit
